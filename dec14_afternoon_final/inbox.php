...
<?php
	...
	$keywords = preg_split("/[\s,%20]+/",$querystring);
	foreach($keywords as $querystring)
	{
		$len++;
	}
?>
...
{	
 echo $_POST['keyword'];
 $srch="Location:http://pba.cs.clemson.edu/~f09t10/Project/Milestone1/inbox.php?".$_POST['keyword'];
 header($srch);
}
echo '<table border="1">';
...
if($querystring == '')
{
$query= "select * from messages where sendto='$sendto' AND flag='0'";
...
{	
$check = 0;
$q="select distinct email from registered_users where (fname='$querystring' OR lname='$querystring')";
$r=mysql_query($q);
while($r1=mysql_fetch_array($r))
{
if($bit == 1)
	{
	 $mail[]=$r1['email']; //$mail has Apple's email appleap@clemson.edu
	}
	else
	{
	 $maill[]=$r1['email'];
...
if($len == 2)
	{
    $final=array_intersect($mail, $maill);
...
//    $from[]=$final;
 ...
 if($from <> '')
 {	
 $query="select * from messages where sendto='$sendto' AND (MATCH(subject,body) AGAINST ('$querystring' in boolean mode) OR sendfrom='$from')  AND flag='0'";
 $result=mysql_query($query);
 }
while($rows=mysql_fetch_array($result))
{	
	$coun=0;
	$array[]=$rows['message_id'];
	foreach($array as $t)
		{
				if($t == $rows['message_id'])
				$coun++;
		}
	$sendfrom =$rows['sendfrom'];
	$query1= "select * from registered_users where email='$sendfrom'";
...
if($check == 0)
{
  $query="select * from messages where sendto='$sendto' AND (subject regexp '^$querystring | $querystring | $querystring$| $querystring$|^$querystring$|^$querystring$|$querystring,' OR body regexp '^$querystring | $querystring | $querystring$| $querystring$|^$querystring$|^$querystring$|$querystring,')  AND flag='0'";
  $result=mysql_query($query);
while($rows=mysql_fetch_array($result))
{	
